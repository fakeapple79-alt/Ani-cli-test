...
The Docker image also installs a pinned `curl-impersonate` release and exposes `curl_chrome116`/`curl_ff117`, which current ani-cli and the search adapter use when AniDB blocks ordinary curl with HTTP 403. After the first deployment, create or copy the service’s public HTTPS domain. Then set these service variables:
...
If search returns no titles, the provider’s HTML layout may have changed or may still be blocking the Railway IP. The search adapter uses browser-impersonating curl, while final resolution uses ani-cli’s same browser-impersonating curl fallback. Treat both as upstream adapters that may need maintenance.
