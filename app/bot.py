    ...
    if context is not None:
        ...
        if watching:
            ...
            text += (
                ...
                f"{escape(str(selected['title']))} — Episode {watching['episode']}"
            )
    return text
    ...
    if watching:
        ...
        rows.append([
            InlineKeyboardButton(
                f"▶  Continue {selected['title'][:28]} — Episode {watching['episode']}",
...
def _episode_keyboard(page: int = 0) -> InlineKeyboardMarkup:
    start = page * 20 + 1
    rows = []
    for offset in range(0, 20, 5):
        rows.append(
            [
                ...
                for episode in range(start + offset, start + offset + 5)
            ]
        )
    navigation = []
    if page > 0:
        navigation.append(InlineKeyboardButton("‹  Previous", callback_data="episodes:prev"))
    navigation.append(InlineKeyboardButton(f"Page {page + 1}", callback_data="episodes:noop"))
...
def _binge_keyboard(episode: int) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    if episode < 9999:
...
def _source_keyboard(episode: int, url: str) -> InlineKeyboardMarkup:
    rows = [
...
async def pick_result(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query is None or query.message is None:
        return
    await query.answer("Loading anime card…")
    ...
    await query.edit_message_text(
        ...
        parse_mode=ParseMode.HTML,
        reply_markup=_episode_keyboard(0),
    )
...
async def _resolve_episode(
    ...
    if episode < 1 or episode > 9999:
        await message.reply_text("Please choose an episode between 1 and 9999.")
        return

    if from_callback:
        work_message = message
        await work_message.edit_text(f"<b>Fetching Episode {episode}... ✨</b>", parse_mode=ParseMode.HTML)
    else:
        work_message = await message.reply_text(
            f"<b>Fetching Episode {episode}... ✨</b>", parse_mode=ParseMode.HTML
        )

    stop, task = await _start_animation(
        work_message,
        [
            f"Fetching Episode {episode}... ✨",
    ...
    try:
        ...
        final = episode >= 1000 or "final" in error_text or "last episode" in error_text
        if final:
            body = (
    ...
    await work_message.edit_text(
        ...
        "<b>Continue watching?</b>",
        parse_mode=ParseMode.HTML,
        reply_markup=_source_keyboard(episode, url),
    )
...
async def episode_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "episodes:next":
        context.user_data["episode_page"] = min(49, context.user_data.get("episode_page", 0) + 1)
        await query.edit_message_reply_markup(_episode_keyboard(context.user_data["episode_page"]))
        return
    ...
    if data == "episodes:show":
        context.user_data["ui_state"] = "episodes"
        await query.edit_message_text(_selected_text(selected), parse_mode=ParseMode.HTML, reply_markup=_episode_keyboard(context.user_data.get("episode_page", 0)))
        return
...
async def list_item_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data["episode_page"] = 0
    context.user_data["ui_state"] = "episodes"
    await query.edit_message_text(_selected_text(selected), parse_mode=ParseMode.HTML, reply_markup=_episode_keyboard(0))
